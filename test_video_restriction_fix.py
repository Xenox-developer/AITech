#!/usr/bin/env python3
"""
Тест для проверки исправления ограничений видео файлов
"""
...
def test_video_restrictions():
    """Тест проверки ограничений видео для разных планов"""
    
    print("🧪 Тестирование ограничений видео файлов...")
    ...
    try:
        ...
        for user_id, email, plan in test_users:
            allowed, message = subscription_manager.check_video_support(user_id)
            results[plan] = {
                'allowed': allowed,
                'message': message,
                'expected_allowed': SUBSCRIPTION_PLANS[plan].video_support
            }
            
            print(f"📋 План {plan.upper()}:")
            print(f"   Ожидается: {'✅ Разрешено' if SUBSCRIPTION_PLANS[plan].video_support else '❌ Запрещено'}")
            print(f"   Результат: {'✅ Разрешено' if allowed else '❌ Запрещено'}")
            if message:
                print(f"   Сообщение: {message}")
            
            # Проверяем корректность
            if allowed == SUBSCRIPTION_PLANS[plan].video_support:
        ...
        if all_tests_passed:
            print("🎉 ВСЕ ТЕСТЫ ПРОЙДЕНЫ! Ограничения видео работают корректно.")
            
            # Выводим сводку по планам
            print("\n📊 Сводка по планам:")
            print("FREEMIUM: ❌ Видео запрещено")
            print("LITE:     ❌ Видео запрещено") 
            print("STARTER:  ✅ Видео разрешено")
            print("BASIC:    ✅ Видео разрешено")
            print("PRO:      ✅ Видео разрешено")
...
def test_error_messages():
    ...
    try:
        ...
        # Создаем пользователя с планом freemium (без поддержки видео)
        c.execute('''
            INSERT INTO users (id, email, subscription_type)
        ...
        allowed, message = subscription_manager.check_video_support(1)
        
        expected_message = "Загрузка видео файлов доступна только в планах STARTER, BASIC и PRO. Обновите план для продолжения."
...
def test_plan_configuration():
    ...
    for plan, expected in expected_video_support.items():
        if plan in SUBSCRIPTION_PLANS:
            actual = SUBSCRIPTION_PLANS[plan].video_support
...
if __name__ == '__main__':
    print("🚀 Запуск тестов исправления ограничений видео...")
    print("=" * 60)
    
    # Запускаем тесты
    test1_passed = test_plan_configuration()
    test2_passed = test_video_restrictions()
    test3_passed = test_error_messages()
    
    print("\n" + "=" * 60)
    if test1_passed and test2_passed and test3_passed:
        print("🎉 ВСЕ ТЕСТЫ ПРОЙДЕНЫ! Исправление работает корректно.")
        print("\n✅ Теперь пользователи с планами FREE и LITE не смогут загружать видео файлы")
        print("✅ Пользователи с планами STARTER, BASIC и PRO могут загружать видео файлы")
        print("✅ Сообщения об ошибках корректны")
        print("✅ Интерфейс адаптируется под план пользователя")
    else:
